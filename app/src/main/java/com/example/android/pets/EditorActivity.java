...
public class EditorActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Examine the intent that was used to create this activity to figure out if we have to Add a new Pet or
        // Edit a pre-existing Pet
        Intent intent = getIntent();
        mCurrentPetUri = intent.getData();

        // If the Intent doesn't contain the URI then we know we have to Add A Pet
        if (mCurrentPetUri == null) {
            // We change the App bar to "Add a Pet"
            setTitle("Add Pet");
            ...
            // We change the App Bar to read "Edit Pet"
            setTitle("Edit Pet");
    ...
    private void savePet() {
        ...
        // Checks if Entry in Weight field is Empty. If it is then by default it value turns to 0
        ...
        // PetsDbHelper helperDb = new PetsDbHelper(this);
        // Gets the database in write mode
        //SQLiteDatabase db = helperDb.getWritableDatabase();
        // Insert a new row for pet in the database, returning the ID of that new row.
        //long petsId = db.insert(PetContract.PetEntry.TABLE_NAME,null,values);

        // Show a toast message depending on whether or not the insertion was successful
        /*if(petsId!=-1){
            Toast.makeText(this,"Pet Saved with id = "+petsId,Toast.LENGTH_SHORT).show();
        }
        else{
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.action_delete:
                //Show Dialog asking for confirmation to Delete Pet
    ...
    public void onLoadFinished(@NonNull Loader<Cursor> loader, Cursor data) {
        ...
        if (data.moveToFirst()) {
            ...
            String PetName = data.getString(data.getColumnIndex(PetContract.PetEntry.COLUMN_PET_NAME));
            String BreedName = data.getString(data.getColumnIndex(PetContract.PetEntry.COLUMN_PET_BREED));
